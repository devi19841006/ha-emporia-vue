...
from .const import DOMAIN, ENABLE_1S, ENABLE_1D, ENABLE_1M, ENABLE_1MON, VUE_DATA
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        async def async_update_data_1sec() -> dict:
            """Fetch data from API endpoint at a 1 second interval.

            This is the place to pre-process the data to lookup tables
            so entities can quickly look up their data.
            """
            data: dict = await update_sensors(vue, [Scale.SECOND.value])
